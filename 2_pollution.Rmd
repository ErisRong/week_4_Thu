...
# Your code below:
library(ggplot2)
EBE_pollution=ggplot(data = Madrid,aes(x=date,y=EBE))+
  geom_point()
...
madrid_pollution <- Madrid %>% 
  group_by(year, mnth) %>% 
  summarise(maxEBE = max(EBE,na.rm = TRUE))
```
...
# Your code below:
#检查mnth的名字排列是否有顺序
levels(madrid_pollution$mnth)
#结果出现的是NULL，代表这些名字只是单纯的名字而不是元素，所以我们需要把他们转换成元素，并按顺序排列，转换元素运用的是“factor”代码，排列顺序用的是“level”代码
madrid_pollution$mnth <- factor(madrid_pollution$mnth, 
                                 levels = month.name)
                
madrid_plot=ggplot(data = madrid_pollution,aes(x=mnth,y=maxEBE))+
  geom_point() +
  facet_wrap(~year)#“～year”意味着将所有的结果按照year来分类成不同的parts
#以上的步骤只显示了数据，并没有做出图像，想要显示图像我们需要print此数据，然后用“theme"代码让他们倾斜45度
madrid_plot+
...
```{r subtract-dfs}
# Your code below:
pollution_2001_2006 = full_join(pollution_2001, pollution_2006) %>% 
  summarise(pollution_2001 - pollution_2006) %>% 
  select(-TOL) %>% 
  mutate(max_pollutant = max(pollution_2001_2006,na.rm = TRUE))
 #the result is NOx

max(pollution_2001_2006,na.rm = TRUE)
...
#2006减去2001数据库的得数是负数，用·“min”代码运行的逻辑是：我们想要得到这些负值中最小的数值，也就是最负的数值，也就是减少最多的那个数值
```
...
```{r pollution_diff}
# Your code below:
pollution_diff=pollution_2001_2006 %>% 
  pivot_longer(cols = everything(), names_to = "Pollutant", values_to = "Data")
```
